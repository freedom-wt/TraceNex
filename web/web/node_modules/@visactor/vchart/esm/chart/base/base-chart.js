...
import { isFunction, isEmpty, isNil, isString, isEqual } from "@visactor/vutils";
...
export class BaseChart extends CompilableBase {
    ...
    init() {
        ...
        }))), this._initEvent(), this._canStack && (this._stack = new Stack(this), this._stack.init()), 
        this.reDataFlow();
    }
    reDataFlow() {
    ...
    _createBackground() {
        ...
        backgroundStyle && (this._backgroundMark = Factory.createMark("group", "chart-background", {
            model: this,
    ...
    layout(params) {
        ...
        this.getLayoutTag()) {
            ...
            this._layoutFunc(this, elements, this._layoutRect, this._viewBox), this._event.emit(ChartEvent.afterLayout, {
                elements: elements
            }), this.setLayoutTag(!1), this.onLayoutEnd(params), this._event.emit(ChartEvent.layoutEnd, {
                chart: this
            });
        }
    ...
    setSelected(datum, filter, region) {
        this._setStateInDatum(STATE_VALUE_ENUM.STATE_SELECTED, !0, datum, filter, region);
    }
    setHovered(datum, filter, region) {
        this._setStateInDatum(STATE_VALUE_ENUM.STATE_HOVER, !0, datum, filter, region);
    }
    _initEvent() {
        [ ChartEvent.dataZoomChange, ChartEvent.scrollBarChange ].forEach((event => {
            this._event.on(event, (({value: value}) => {
                ...
                const enableMarkAnimate = () => {
                    this._enableMarkAnimation([ "exit", "update" ]), this._event.off(VGRAMMAR_HOOK_EVENT.AFTER_MARK_RENDER_END, enableMarkAnimate);
                };
    ...
    _setStateInDatum(stateKey, checkReverse, datum, filter, region) {
        const keys = (datum = datum ? array(datum) : null) ? Object.keys(datum[0]) : null;
