...
var renderKatex = __name(async (text2, config2) => {
  ...
  const { default: katex } = await import("./katex-OJ27PIWW.js");
...
function formatDate2(date2) {
  var hours = date2.getUTCHours(), minutes = date2.getUTCMinutes(), seconds2 = date2.getUTCSeconds(), milliseconds2 = date2.getUTCMilliseconds();
  return isNaN(date2) ? "Invalid Date" : formatYear3(date2.getUTCFullYear(), 4) + "-" + pad3(date2.getUTCMonth() + 1, 2) + "-" + pad3(date2.getUTCDate(), 2) + (milliseconds2 ? "T" + pad3(hours, 2) + ":" + pad3(minutes, 2) + ":" + pad3(seconds2, 2) + "." + pad3(milliseconds2, 3) + "Z" : seconds2 ? "T" + pad3(hours, 2) + ":" + pad3(minutes, 2) + ":" + pad3(seconds2, 2) + "Z" : minutes || hours ? "T" + pad3(hours, 2) + ":" + pad3(minutes, 2) + "Z" : "");
}
function dsv_default2(delimiter) {
  ...
  function parseRows(text2, f) {
    var rows = [], N = text2.length, I = 0, n = 0, t, eof = N <= 0, eol = false;
    if (text2.charCodeAt(N - 1) === NEWLINE2) --N;
    if (text2.charCodeAt(N - 1) === RETURN2) --N;
    function token() {
      if (eof) return EOF2;
      ...
      if (text2.charCodeAt(j) === QUOTE2) {
        ...
        if ((i = I) >= N) eof = true;
        else if ((c3 = text2.charCodeAt(I++)) === NEWLINE2) eol = true;
        else if (c3 === RETURN2) {
          eol = true;
          if (text2.charCodeAt(I) === NEWLINE2) ++I;
        }
        return text2.slice(j + 1, i - 1).replace(/""/g, '"');
      }
      while (I < N) {
        if ((c3 = text2.charCodeAt(i = I++)) === NEWLINE2) eol = true;
        else if (c3 === RETURN2) {
          eol = true;
          if (text2.charCodeAt(I) === NEWLINE2) ++I;
        } else if (c3 !== DELIMITER) continue;
    ...
    while ((t = token()) !== EOF2) {
      var row = [];
      while (t !== EOL2 && t !== EOF2) row.push(t), t = token();
      if (f && (row = f(row, n++)) == null) continue;
      rows.push(row);
    }
    return rows;
...
function cartesian(spherical2) {
  var lambda = spherical2[0], phi = spherical2[1], cosPhi = cos3(phi);
...
function polygonContains_default(polygon, point6) {
  var lambda = longitude(point6), phi = point6[1], sinPhi = sin3(phi), normal = [sin3(lambda), -cos3(lambda), 0], angle = 0, winding = 0;
  var sum4 = new Adder();
  if (sinPhi === 1) phi = halfPi4 + epsilon7;
  else if (sinPhi === -1) phi = -halfPi4 - epsilon7;
...
function clipAntimeridianInterpolate(from, to, direction, stream) {
  var phi;
  if (from == null) {
    phi = direction * halfPi4;
    stream.point(-pi5, phi);
    stream.point(0, phi);
    stream.point(pi5, phi);
    stream.point(pi5, 0);
    stream.point(pi5, -phi);
...
function mercatorRaw(lambda, phi) {
  return [lambda, log4(tan((halfPi4 + phi) / 2))];
}
...
function equalEarthRaw(lambda, phi) {
  var l = asin2(M * sin3(phi)), l2 = l * l, l6 = l2 * l2 * l2;
  return [
...
function transverseMercatorRaw(lambda, phi) {
  return [log4(tan((halfPi4 + phi) / 2)), -lambda];
}
...
//# sourceMappingURL=chunk-VLKDY5LG.js.map
