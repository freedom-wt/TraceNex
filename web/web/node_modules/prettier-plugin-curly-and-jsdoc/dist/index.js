...
// ../../node_modules/.pnpm/binary-searching@2.0.5/node_modules/binary-searching/search-bounds.js
var require_search_bounds = __commonJS({
  "../../node_modules/.pnpm/binary-searching@2.0.5/node_modules/binary-searching/search-bounds.js"(exports, module) {
    "use strict";
...
// ../../node_modules/.pnpm/prettier-plugin-jsdoc@1.3.0_prettier@3.0.3/node_modules/prettier-plugin-jsdoc/dist/index.js
var dist_exports = {};
...
// ../../node_modules/.pnpm/comment-parser@1.4.0/node_modules/comment-parser/es6/primitives.js
var Markers;
(function(Markers2) {
  Markers2["start"] = "/**";
  Markers2["nostart"] = "/***";
...
// ../../node_modules/.pnpm/comment-parser@1.4.0/node_modules/comment-parser/es6/util.js
...
// ../../node_modules/.pnpm/comment-parser@1.4.0/node_modules/comment-parser/es6/parser/block-parser.js
var reTag = /^@\S+/;
...
// ../../node_modules/.pnpm/comment-parser@1.4.0/node_modules/comment-parser/es6/parser/source-parser.js
...
// ../../node_modules/.pnpm/comment-parser@1.4.0/node_modules/comment-parser/es6/parser/spec-parser.js
function getParser3({ tokenizers: tokenizers2 }) {
  return function parseSpec(source) {
    var _a;
    let spec = seedSpec({ source });
    for (const tokenize of tokenizers2) {
      spec = tokenize(spec);
...
// ../../node_modules/.pnpm/comment-parser@1.4.0/node_modules/comment-parser/es6/parser/tokenizers/tag.js
...
// ../../node_modules/.pnpm/comment-parser@1.4.0/node_modules/comment-parser/es6/parser/tokenizers/type.js
...
// ../../node_modules/.pnpm/comment-parser@1.4.0/node_modules/comment-parser/es6/parser/tokenizers/name.js
...
// ../../node_modules/.pnpm/comment-parser@1.4.0/node_modules/comment-parser/es6/parser/tokenizers/description.js
...
// ../../node_modules/.pnpm/comment-parser@1.4.0/node_modules/comment-parser/es6/parser/index.js
function getParser4({ startLine = 0, fence = "```", spacing = "compact", markers = Markers, tokenizers: tokenizers2 = [
  tagTokenizer(),
  ...
  const parseSpec = getParser3({ tokenizers: tokenizers2 });
...
// ../../node_modules/.pnpm/comment-parser@1.4.0/node_modules/comment-parser/es6/stringifier/index.js
...
// ../../node_modules/.pnpm/comment-parser@1.4.0/node_modules/comment-parser/es6/stringifier/inspect.js
var zeroWidth = {
  line: 0,
  start: 0,
  delimiter: 0,
...
// ../../node_modules/.pnpm/comment-parser@1.4.0/node_modules/comment-parser/es6/index.js
...
// ../../node_modules/.pnpm/prettier-plugin-jsdoc@1.3.0_prettier@3.0.3/node_modules/prettier-plugin-jsdoc/dist/utils.js
var import_binary_searching = __toESM(require_search_bounds(), 1);
import { format } from "prettier";

// ../../node_modules/.pnpm/prettier-plugin-jsdoc@1.3.0_prettier@3.0.3/node_modules/prettier-plugin-jsdoc/dist/tags.js
...
var PARAM = "param";
var PRIVATE = "private";
var PRIVATE_REMARKS = "privateRemarks";
...
// ../../node_modules/.pnpm/prettier-plugin-jsdoc@1.3.0_prettier@3.0.3/node_modules/prettier-plugin-jsdoc/dist/roles.js
var TAGS_SYNONYMS = {
  arg: PARAM,
  argument: PARAM,
  const: CONSTANT,
...
var TAGS_DEFAULT = [DEFAULT, DEFAULT_VALUE];
var TAGS_NAMELESS = [
  BORROWS,
  CATEGORY,
  DEPRECATED,
...
var TAGS_PEV_FORMATE_DESCRIPTION = [
  BORROWS,
  ...TAGS_DEFAULT,
  MEMBEROF,
  MODULE,
...
var TAGS_VERTICALLY_ALIGN_ABLE = [
  EXTENDS,
  PARAM,
  PROPERTY,
  RETURNS,
...
var TAGS_GROUP_HEAD = [CALLBACK, TYPEDEF];
var TAGS_GROUP_CONDITION = [
  ...TAGS_GROUP_HEAD,
  TYPE,
  PROPERTY,
  PARAM,
...
function withoutStrings(type, mapFn) {
  ...
  return modifiedType.replace(/String\$(\d+)\$/g, (_, index2) => strings[index2]);
}
async function formatType(type, options2) {
  try {
...
function addStarsToTheBeginningOfTheLines(originalComment, comment, options2) {
  if (options2.jsdocCommentLineStrategy === "singleLine" && numberOfAStringInString(comment.trim(), "\n") === 0 || options2.jsdocCommentLineStrategy === "keep" && numberOfAStringInString(originalComment, "\n") === 0) {
    return `* ${comment.trim()} `;
  }
  return `*${comment.replace(/(\n(?!$))/g, "\n * ")}
 `;
}
function numberOfAStringInString(string3, search2) {
  return (string3.match(new RegExp(search2, "g")) || []).length;
}
...
function detectEndOfLine(text3) {
  const counter = {
    "\r": 0,
    "\r\n": 0,
    "\n": 0
  ...
  let m;
  while (m = lineEndPattern.exec(text3)) {
    counter[m[0]]++;
  }
  const cr = counter["\r"];
  const crlf2 = counter["\r\n"];
  const lf = counter["\n"];
  const max = Math.max(cr, crlf2, lf);
  if (lf === max) {
    return "lf";
  } else if (crlf2 === max) {
    return "crlf";
...
function findTokenIndex(tokens, token) {
  return import_binary_searching.default.eq(tokens, token, (a, b) => {
...
async function formatCode(result, beginningSpace, options2) {
  const { printWidth, jsdocKeepUnParseAbleExampleIndent } = options2;
  if (result.split("\n").slice(1).every((v) => !v.trim() || v.startsWith(beginningSpace))) {
...
var findPluginByParser = (parserName, options2) => {
  const tsPlugin = options2.plugins.find((plugin) => {
...
var isDefaultTag = (tag) => TAGS_DEFAULT.includes(tag);

// ../../node_modules/.pnpm/prettier-plugin-jsdoc@1.3.0_prettier@3.0.3/node_modules/prettier-plugin-jsdoc/dist/descriptionFormatter.js
...
function fromMarkdown(value, encoding, options2) {
  ...
  return compiler(options2)(
    postprocess(
      parse2(options2).document().write(preprocess()(value, encoding, true))
    )
  );
}
...
// ../../node_modules/.pnpm/prettier-plugin-jsdoc@1.3.0_prettier@3.0.3/node_modules/prettier-plugin-jsdoc/dist/descriptionFormatter.js
var TABLE = "2@^5!~#sdE!_TABLE";
var parserSynonyms = (lang) => {
  switch (lang) {
    case "js":
    case "javascript":
    case "jsx":
...
async function formatDescription(tag, text3, options2, formatOptions) {
  if (!text3)
    return text3;
  const { printWidth } = options2;
  const { tagStringLength = 0, beginningSpace } = formatOptions;
  text3 = text3.replace(/^(\d+)[-][\s|]+/g, "$1. ");
  text3 = text3.replace(/\n+(\s*\d+)[-][\s]+/g, "\n$1. ");
  const fencedCodeBlocks = text3.matchAll(/```\S*?\n[\s\S]+?```/gm);
  const indentedCodeBlocks = text3.matchAll(/^\r?\n^(?:(?:(?:[ ]{4}|\t).*(?:\r?\n|$))+)/gm);
  const allCodeBlocks = [...fencedCodeBlocks, ...indentedCodeBlocks];
  const tables = [];
  text3 = text3.replace(/((\n|^)\|[\s\S]*?)((\n[^|])|$)/g, (code, _1, _2, _3, _, offs) => {
    for (const block of allCodeBlocks) {
      if (block.index !== void 0 && block.index <= offs + 1 && offs + code.length + 1 <= block.index + block[0].length) {
        return code;
      }
    }
...
${_3 ? _3.slice(1) : ""}`;
  ...
  if (options2.jsdocCapitalizeDescription && !TAGS_PEV_FORMATE_DESCRIPTION.includes(tag)) {
    text3 = capitalizer(text3);
  }
  text3 = `${tagStringLength ? `${"!".repeat(tagStringLength - 1)}?` : ""}${text3.startsWith("```") ? "\n" : ""}${text3}`;
  let tableIndex = 0;
  text3 = text3.replace(new RegExp(`\\n[ ]{${beginningSpace.length}}`, "g"), "\n");
  const rootAst = fromMarkdown(text3);
  async function stringifyASTWithoutChildren(mdAst, intention, parent) {
    if (mdAst.type === "inlineCode") {
      return `\`${mdAst.value}\``;
    }
    if (mdAst.type === "code") {
      let result2 = mdAst.value || "";
      let _intention = intention;
      if (result2) {
        if (mdAst.lang) {
          const supportParsers = parserSynonyms(mdAst.lang.toLowerCase());
          const parser = supportParsers?.includes(options2.parser) ? options2.parser : supportParsers?.[0] || mdAst.lang;
          result2 = await formatCode(result2, intention, {
            ...options2,
            parser,
            jsdocKeepUnParseAbleExampleIndent: true
          });
        } else if (options2.jsdocPreferCodeFences || false) {
          result2 = await formatCode(result2, _intention, {
            ...options2,
            jsdocKeepUnParseAbleExampleIndent: true
          });
        } else {
      ...
      const addFence = options2.jsdocPreferCodeFences || !!mdAst.lang;
      result2 = addFence ? result2 : result2.trimEnd();
      return result2 ? addFence ? `

${_intention}\`\`\`${mdAst.lang || ""}${result2}\`\`\`` : `
${result2}` : "";
    }
    if (mdAst.value === TABLE) {
      ...
      if (tables.length > 0) {
        let result2 = tables?.[tableIndex] || "";
        tableIndex++;
        if (result2) {
          result2 = (await format2(result2, {
            ...options2,
            parser: "markdown"
          })).trim();
        }
...
${intention}${result2.split("\n").join(`
${intention}`)}` : mdAst.value}`;
      }
    }
    if (mdAst.type === "break") {
      return `\\
`;
    ...
    return mdAst.value || mdAst.title || mdAst.alt || "";
  }
  async function stringyfy(mdAst, intention, parent) {
    ...
    return (await Promise.all(mdAst.children.map(async (ast, index2) => {
      switch (ast.type) {
        case "listItem": {
          let _listCount = `
${intention}- `;
          if (typeof mdAst.start === "number") {
            const count = index2 + (mdAst.start ?? 1);
            _listCount = `
${intention}${count}. `;
          }
          const _intention = intention + " ".repeat(_listCount.length - 1);
          const result2 = (await stringyfy(ast, _intention, mdAst)).trim();
          return `${_listCount}${result2}`;
        }
        case "list": {
          let end = "";
          if (tag !== DESCRIPTION && mdAst.type === "root" && index2 === mdAst.children.length - 1) {
            end = "\n";
          }
          return `
...
${paragraph.split("\\\n").map((_paragraph) => {
            const links = [];
            _paragraph = _paragraph.replace(/{@(link|linkcode|linkplain)[\s](([^{}])*)}/g, (_, tag2, link) => {
              links.push(link);
              return `{@${tag2}${"_".repeat(link.length)}}`;
            });
            _paragraph = _paragraph.replace(/\s+/g, " ");
            if (options2.jsdocCapitalizeDescription && !TAGS_PEV_FORMATE_DESCRIPTION.includes(tag))
              _paragraph = capitalizer(_paragraph);
            if (options2.jsdocDescriptionWithDot)
              _paragraph = _paragraph.replace(/([\w\p{L}])$/u, "$1.");
            let result2 = breakDescriptionToLines(_paragraph, printWidth, intention);
            result2 = result2.replace(/{@(link|linkcode|linkplain)([_]+)}/g, (original, tag2, underline) => {
              const link = links[0];
...
[${ast.label}]: ${ast.url}`;
        ...
        case "blockquote": {
          ...
          return `${intention}> ${paragraph.trim().replace(/(\n+)/g, `$1${intention}> `)}`;
        }
      }
...
// ../../node_modules/.pnpm/prettier-plugin-jsdoc@1.3.0_prettier@3.0.3/node_modules/prettier-plugin-jsdoc/dist/stringify.js
var stringify2 = async ({ name, description, type, tag }, tagIndex, finalTagsArray, options2, maxTagTitleLength, maxTagTypeNameLength, maxTagNameLength) => {
  let tagString = "\n";
  if (tag === SPACE_TAG_DATA.tag) {
    return tagString;
  }
  const { printWidth, jsdocSpaces, jsdocVerticalAlignment, jsdocDescriptionTag, tsdoc, useTabs, tabWidth, jsdocSeparateTagGroups } = options2;
  ...
  if (tag === EXAMPLE && !tsdoc) {
    ...
    const formattedExample = await formatCode(description, beginningSpace, options2);
    tagString += formattedExample.replace(new RegExp(`^\\n${beginningSpace.replace(/[\t]/g, "[\\t]").replace(/[^S\r\n]/g, "[^S\\r\\n]")}\\n`), "").trimEnd();
  } else if (description) {
    let descriptionString = "";
    if (useTagTitle)
      tagString += gap + " ".repeat(descGapAdj);
    if (TAGS_PEV_FORMATE_DESCRIPTION.includes(tag) || !TAGS_ORDER[tag]) {
      descriptionString = description;
    } else {
      const [, firstWord] = /^\s*(\S+)/.exec(description) || ["", ""];
      const beginningSpace = tag === DESCRIPTION || [EXAMPLE, REMARKS, PRIVATE_REMARKS].includes(tag) && tsdoc ? "" : "  ";
      if (tag !== DESCRIPTION && tagString.length + firstWord.length > printWidth || [REMARKS, PRIVATE_REMARKS].includes(tag)) {
        descriptionString = `
${beginningSpace}` + await formatDescription(tag, description, options2, {
          beginningSpace
        });
      } else {
        descriptionString = await formatDescription(tag, description, options2, {
...
// ../../node_modules/.pnpm/prettier-plugin-jsdoc@1.3.0_prettier@3.0.3/node_modules/prettier-plugin-jsdoc/dist/parser.js
var { name: nameTokenizer2, tag: tagTokenizer2, type: typeTokenizer2, description: descriptionTokenizer2 } = tokenizers;
var getParser5 = (originalParse, parserName) => async function jsdocParser(text3, parsersOrOptions, maybeOptions) {
  let options2 = maybeOptions ?? parsersOrOptions;
  const prettierParse = findPluginByParser(parserName, options2)?.parse || originalParse;
  const ast = prettierParse(text3, options2);
  options2 = {
    ...options2,
    printWidth: options2.jsdocPrintWidth ?? options2.printWidth
  };
  const eol = options2.endOfLine === "auto" ? detectEndOfLine(text3) : options2.endOfLine;
  options2 = { ...options2, endOfLine: "lf" };
  await Promise.all(ast.comments.map(async (comment) => {
    ...
    const parsed = parse(commentString, {
      ...
      tokenizers: [
        ...
        (spec) => {
          ...
          return typeTokenizer2("preserve")(spec);
        },
        nameTokenizer2(),
        descriptionTokenizer2("preserve")
      ]
    })[0];
    comment.value = "";
    if (!parsed) {
    ...
    const commentContentPrintWidth = getIndentationWidth(comment, text3, options2);
    ...
    tags = sortTags(tags, paramsOrder, options2);
    if (options2.jsdocSeparateReturnsFromParam) {
      tags = tags.flatMap((tag, index2) => {
        if (tag.tag === RETURNS && tags[index2 - 1]?.tag === PARAM) {
          return [SPACE_TAG_DATA, tag];
        }
        return [tag];
      });
    }
    if (options2.jsdocAddDefaultToDescription) {
      tags = tags.map(addDefaultValueToDescription);
    }
    tags = await Promise.all(tags.map(assignOptionalAndDefaultToName).map(async ({ type, ...rest }) => {
      if (type) {
    ...
    if (options2.jsdocSeparateTagGroups) {
      tags = tags.flatMap((tag, index2) => {
        const prevTag = tags[index2 - 1];
        if (prevTag && prevTag.tag !== DESCRIPTION && prevTag.tag !== EXAMPLE && prevTag.tag !== SPACE_TAG_DATA.tag && tag.tag !== SPACE_TAG_DATA.tag && prevTag.tag !== tag.tag) {
          return [SPACE_TAG_DATA, tag];
        }
        return [tag];
      });
    ...
    for (const [tagIndex, tagData] of filteredTags.entries()) {
      const formattedTag = await stringify2(tagData, tagIndex, filteredTags, { ...options2, printWidth: commentContentPrintWidth }, maxTagTitleLength, maxTagTypeLength, maxTagNameLength);
      comment.value += formattedTag;
    }
    comment.value = comment.value.trimEnd();
    if (comment.value) {
      comment.value = addStarsToTheBeginningOfTheLines(originalValue, comment.value, options2);
    }
    if (eol === "cr") {
...
function sortTags(tags, paramsOrder, options2) {
  let canGroupNextTags = false;
  let shouldSortAgain = false;
  tags = tags.reduce((tagGroups, cur) => {
    if (tagGroups.length === 0 || TAGS_GROUP_HEAD.includes(cur.tag) && canGroupNextTags) {
    ...
    return tagGroups;
  }, []).flatMap((tagGroup, index2, array) => {
    tagGroup.sort((a, b) => {
      if (paramsOrder && paramsOrder.length > 1 && a.tag === PARAM && b.tag === PARAM) {
      ...
      return getTagOrderWeight(a.tag, options2) - getTagOrderWeight(b.tag, options2);
    });
    if (array.length - 1 !== index2) {
      tagGroup.push(SPACE_TAG_DATA);
    }
    if (index2 > 0 && tagGroup[0]?.tag && !TAGS_GROUP_HEAD.includes(tagGroup[0].tag)) {
      shouldSortAgain = true;
    }
    return tagGroup;
  });
  return shouldSortAgain ? sortTags(tags, paramsOrder, options2) : tags;
}
function getTagOrderWeight(tag, options2) {
  if (tag === DESCRIPTION && !options2.jsdocDescriptionTag) {
    return -1;
  }
  let index2;
  if (options2.jsdocTagsOrder?.[tag] !== void 0) {
    index2 = options2.jsdocTagsOrder[tag];
  } else {
    index2 = TAGS_ORDER[tag];
  }
  return index2 === void 0 ? TAGS_ORDER.other : index2;
}
function isBlockComment(comment) {
  return comment.type === "CommentBlock" || comment.type === "Block";
}
function getIndentationWidth(comment, text3, options2) {
  ...
  return options2.printWidth - (spaces + tabs * options2.tabWidth) - " * ".length;
}
var TAGS_ORDER_ENTRIES = Object.entries(TAGS_ORDER);
...
function getParamsOrders(ast, tokenIndex) {
  ...
  try {
    ...
    if (nextTokenType.label === "const") {
      ...
      if (typeof arrowItem?.type === "object" && arrowItem.type.label === "=>") {
        params = tokensAfterComment.slice(0, endIndex + 1);
      }
    }
    return params?.filter(({ type }) => typeof type === "object" && type.label === "name").map(({ value }) => value);
  } catch {
  }
  return;
}
...
function mergeParsers(originalParser, parserName) {
  const jsDocParse = getParser5(originalParser.parse, parserName);
  const jsDocPreprocess = (text3, options2) => {
    normalizeOptions(options2);
    const tsPluginParser = findPluginByParser(parserName, options2);
    if (!tsPluginParser) {
      return originalParser.preprocess ? originalParser.preprocess(text3, options2) : text3;
    }
    const preprocess2 = tsPluginParser.preprocess || originalParser.preprocess;
    ...
    return preprocess2 ? preprocess2(text3, options2) : text3;
  };
  const parser = {
    ...originalParser,
    preprocess: jsDocPreprocess,
...
function normalizeOptions(options2) {
  if (options2.jsdocTagsOrder) {
    options2.jsdocTagsOrder = JSON.parse(options2.jsdocTagsOrder);
  }
  if (options2.jsdocCommentLineStrategy) {
    return;
  }
  if (options2.jsdocSingleLineComment) {
    options2.jsdocCommentLineStrategy = "singleLine";
  } else {
    options2.jsdocCommentLineStrategy = "multiline";
  }
}

// src/index.ts
var jsdoc = dist_exports;
var src_default = {
  ...
  parsers: {
    ...
    babel: {
      ...jsdoc.parsers.babel,
      preprocess: (code, options2) => jsdoc.parsers.babel.preprocess(
    ...
    typescript: {
      ...
      preprocess: (code, options2) => jsdoc.parsers.typescript.preprocess(
