      ...
      >
        <Row gutter={16}>
          <Col xs={24} sm={16}>
            <Form.TextArea
              label={t('分组折扣')}
              placeholder={t('为一个 JSON 文本，键为分组名称，值为折扣')}
              extraText={t(
                '分组折扣设置，可以在此处新增分组或修改现有分组的折扣，格式为 JSON 字符串，例如：{"黄金": 0.9, "铂金": 0.89}，表示分组黄金的倍率为 0.8，铂金 分组的倍率为 0.85',
              )}
              field={'GroupRatio'}
        ...
        <Row gutter={16}>
          <Col xs={24} sm={16}>
            <Form.TextArea
              ...
              extraText={t(
                '用户新建令牌时可选的分组，格式为 JSON 字符串，例如：{"黄金": "黄金用户", "铂金": "铂金用户"}，表示用户可以选择黄金用户分组和铂金用户分组',
              )}
        ...
        <Row gutter={16}>
          <Col xs={24} sm={16}>
            <Form.TextArea
              ...
              extraText={t(
                '键为用户分组，值为该用户的特殊分组折扣，例如：{"黄金": {"豆包模型分组": 0. 8}}，表示黄金用户在使用豆包模型分组的令牌时折扣为0.8',
              )}
        ...
        <Row gutter={16}>
          <Col xs={24} sm={16}>
            <Form.TextArea
              ...
              placeholder={t('为一个 JSON 文本')}
              extraText={t(
                '键为用户分组，值为操作映射对象。内层键以"+:"开头表示添加指定分组（键值为分组名称，值为描述），以"-:"开头表示移除指定分组（键值为分组名称），不带前缀的键直接添加该分组。例如：{"黄金": {"+:豆包模型分组": "特殊模型分组", "-":"default":"默认分组"}}，表示黄金用户可以使用豆包模型分组，同时移除 default 分组的访问权限',
              )}
